...
ВАЖЛИВО — СТИЛЬ АДВОКАТА (дотримуйся ТОЧНО):
- НЕ використовуй markdown (###, **, тощо) — пиши чистий текст
- НЕ додавай "Ціна позову", "Судовий збір" у шапку
- НЕ додавай "Дата народження" у шапку — тільки ПІБ, адреса, РНОКПП, тел.
- Посилання на закони у форматі: "ч.2 ст.104 СК України" (без пробілу після ч./ст./п.)
- "Позивач" та "Відповідач" з великої літери
- Закони в лапках: «Про виконавче провадження»
- Прізвище у підписі ВЕЛИКИМИ літерами: "Марія КОВАЛЕНКО"

1. ШАПКА (правий верхній кут, кожен елемент на окремому рядку):
   [Повна назва суду] [область]
   [адреса суду з поштовим індексом]
...
3. ФАБУЛА (виклад обставин справи):
   Детальний хронологічний виклад: дата шлюбу/події, реєстрація, діти, обставини.
   Кожне твердження — окремий абзац.

4. ПРАВОВА ЧАСТИНА:
   Посилання на конкретні статті у форматі: "Відповідно до ч.2 ст.104 СК України..."
   Цитування норм без окремого заголовка "Правові підстави" — вплітай у текст природно.
   Процесуальні повідомлення за ст.175 ЦПК (пп.6-10 ч.3).
   Клопотання про спрощене провадження (ст.274, 276 ЦПК) якщо доречно.

5. ПРОХАЛЬНА ЧАСТИНА:
   "На підставі вищенаведеного, керуючись ст.ст. ... ,"
   (порожній рядок)
   ПРОШУ:
...
6. ДОДАТКИ:
   Додатки:
   Пронумерований перелік (без крапки після номера рядка).

7. ПІДПИС (внизу):
   _________________                       __________________                       [Ім'я ПРІЗВИЩЕ]
   (прізвище ВЕЛИКИМИ літерами)

Всі дати у форматі «ДД місяць РРРР року» (наприклад: 19 травня 2015 року).
НЕ додавай коментарів, пояснень чи markdown — тільки чистий юридичний текст документа.`;
